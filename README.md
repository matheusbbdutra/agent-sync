...
- **`context-window-strategy`** (skill + `tools/cmd/ctx-window/` tool): sliding window + incremental summary for long sessions. Implements the pattern from arXiv [2606.10209v1](https://arxiv.org/html/2606.10209v1) (Microsoft — Lodha et al.) — 91.6% completion vs 71% with full context. Three summarizer paths: own model (default, called via `opencode run --pure` so the running session's LLM generates the summary), Ollama local (privacy), or pure local heuristic (regex fallback). See [ADR](docs/ADR-context-window-strategy.md) for the empirical validation. Auto-compact hook installed in all 5 CLIs (`hooks/ctx-compact.{sh,cursor.sh,antigravity.sh}` + `hooks/ctx-compact.opencode.ts`).
