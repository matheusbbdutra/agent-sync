...
- **`context-window-strategy`** (skill + tool `tools/cmd/ctx-window/`): sliding window + sumário incremental para sessões longas. Implementa o padrão do paper arXiv [2606.10209v1](https://arxiv.org/html/2606.10209v1) (Microsoft — Lodha et al.) — 91.6% de conclusão vs 71% com contexto cheio. Três caminhos de sumarizador: próprio modelo (default, invocado via `opencode run --pure` para que o LLM da sessão gere o sumário), Ollama local (privacidade) ou heurística pura local (fallback). Veja o [ADR](docs/ADR-context-window-strategy.md) para a validação empírica. Hook de auto-compactação instalado nas 5 CLIs (`hooks/ctx-compact.{sh,cursor.sh,antigravity.sh}` + `hooks/ctx-compact.opencode.ts`).
