...
- **D.** ~~`make sync` para propagar correção do adapter `protect-mcp` (STATE.md:161, 169-172) — só documentado, não executado pelo usuário. Verificar se `npx protect-mcp evaluate` retorna `permissionDecision` válido após o adapter.~~ **Fechado 2026-09-19**: nova função `adaptCodexPlugins` em `cmd/agent-sync/hooks.go` varre `~/.codex/plugins/*/hooks/hooks.json` automaticamente em `-apply` e aplica `adaptCodexProtectionHooks` (mesmo helper usado em `syncHookCommandAtEvent`). Cria `<hooks.json>.bak` único na primeira execução (idempotente). Detecta `trusted_hash` em `~/.codex/config.toml` via regex, emite aviso em `stderr` mas prossegue.Próximo passo de validação: rodar `make apply` em ambiente de teste e verificar que os 2 plugins ativos (`protect-mcp`, `review-agent-governance`) foram adaptados e os `.bak` foram criados.
