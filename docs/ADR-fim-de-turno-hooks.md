...
- **Coexistência**: o `agent-react-nudge.cursor.sh` existente em `postToolUse` (lembrete a cada N tool calls) **permanece** — `stop` é complemento, não substituto. `stop` é a rede de segurança final; `postToolUse` é a cadência periódica.
