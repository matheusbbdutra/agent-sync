...
if [ "$((count % THRESHOLD))" -eq 0 ]; then
  ...
  if command -v memory-mcp >/dev/null 2>&1; then
    printf '%s\n' \
      ...
      "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/call\",\"params\":{\"name\":\"record_event\",\"arguments\":{\"agent\":\"claude-code\",\"kind\":\"guard_nudge\",\"note\":\"memory-nudge #$count\",\"session_id\":\"$session_id\"}}}" \
