...
func runSummarize(args []string, stdout, stderr io.Writer) error {
	...
	// chooseHeuristic decide entre o extrator local (grátis, sem LLM) e o
	// LLM da própria CLI da sessão — ver chooseHeuristic. O rótulo mostra o
	// modo resolvido ("auto->heuristic"), não o campo Summarizer gravado na
	// criação da sessão (que fica obsoleto quando o config muda).
	mode := SummarizerFromConfig()
	if chooseHeuristic(mode, s) {
		label := "heuristic"
		if mode == "auto" {
			label = "auto->heuristic"
		}
		return persistSummary(s, "local", label, "", HeuristicExtract(s.Turns).ToYAML(), stdout, stderr)
	}
	...
	return persistSummary(s, cliName, label, promptModel, yaml, stdout, stderr)
}

// autoHeuristicMaxTurns/Chars são os cortes do modo "auto": abaixo deles o
// resumo sai do extrator local (grátis); acima, o volume justifica o juízo
// do LLM. Chars/4 ≈ tokens; 24000 ≈ 6k tokens de histórico. O corte de
// chars é alto de propósito: tool output infla rápido e uma sessão de 5
// turns já passa de 10k chars sem ter "juízo" a exigir LLM.
const (
	autoHeuristicMaxTurns = 20
	autoHeuristicMaxChars = 24000
)

// chooseHeuristic decide se o summarize fica no extrator local (grátis,
// regex de marcadores) ou sobe para o LLM da CLI da sessão.
//   - "heuristic": sempre local.
//   - "auto": local enquanto a sessão é pequena (turns E chars abaixo dos
//     cortes); promove para LLM quando o volume exige julgamento — turns
//     tersos/reversões somem no heurístico, e sessão grande tem ruído
//     suficiente para o custo do LLM valer.
//   - "agent" / "ollama:<modelo>" / desconhecido: LLM (comportamento
//     anterior, mantido como default).
func chooseHeuristic(mode string, s *Session) bool {
	switch mode {
	case "heuristic":
		return true
	case "auto":
		return len(s.Turns) <= autoHeuristicMaxTurns && s.EstimatedChars() <= autoHeuristicMaxChars
	}
	return false
}

// persistSummary grava a versão nova do resumo (session + projeto), imprime
// o resumo de comando e dispara o remember opt-in. Compartilhado entre o
// caminho heurístico e o LLM para não duplicar o tail.
func persistSummary(s *Session, cliName, summarizer, model, yaml string, stdout, stderr io.Writer) error {
	...
	fmt.Fprintf(stdout, "summarized: version %d (previous %d); cli=%s; summarizer=%s; model=%s; turns=%d\n",
		s.Version, prev, cliName, summarizer, model, len(s.Turns))
