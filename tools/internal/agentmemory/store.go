...
var schemaStatements = []string{
	...
	`CREATE UNIQUE INDEX IF NOT EXISTS memories_type_name ON memories(type, name)`,
...
func (s *Store) Upsert(m Memory) error {
	...
	_, err := s.db.Exec(`
		INSERT INTO memories (id, agent, session_id, type, name, description, content, scratch, embedding_json, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, '', ?, ?)
	...
	`, m.ID, m.Agent, m.SessionID, m.Type, m.Name, m.Description, m.Content, m.Scratch, now, now)
	if err != nil {
...
// Get busca uma memória pelo nome (único globalmente por convenção do índice).
func (s *Store) Get(name string) (*Memory, error) {
	row := s.db.QueryRow(`
		SELECT id, agent, session_id, type, name, description, content, scratch, created_at, updated_at
		FROM memories WHERE name = ? LIMIT 1
	`, name)
	var m Memory
	var created, updated string
	if err := row.Scan(&m.ID, &m.Agent, &m.SessionID, &m.Type, &m.Name, &m.Description, &m.Content, &m.Scratch, &created, &updated); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
		return nil, fmt.Errorf("agentmemory: get %s: %w", name, err)
	}
...
func (s *Store) List(agent, typ string, limit int) ([]Memory, error) {
	if limit <= 0 {
		limit = 100
	}
	rows, err := s.db.Query(`
...
func (s *Store) Search(query, agent, typ string, limit int) ([]Memory, error) {
	if limit <= 0 {
		limit = 10
	}
	rows, err := s.db.Query(`
		SELECT m.id, m.agent, m.session_id, m.type, m.name, m.description, m.content, m.scratch, m.created_at, m.updated_at
		FROM memories_fts f
		...
		LIMIT ?
	`, ftsQuery(query), agent, agent, typ, typ, limit)
	if err != nil {
...
func scanMemories(rows *sql.Rows) ([]Memory, error) {
	...
	for rows.Next() {
		var m Memory
		var created, updated string
		if err := rows.Scan(&m.ID, &m.Agent, &m.SessionID, &m.Type, &m.Name, &m.Description, &m.Content, &m.Scratch, &created, &updated); err != nil {
