...
func parseFile(rel, abs string, info os.FileInfo) (*FileEntry, error) {
	...
	entry.Symbols, entry.Imports, entry.Calls, entry.Tables, entry.EnvVars = extractStructured(rel, src)
	return entry, nil
}
...
// extractStructured devolve (symbols, imports, calls, tables, envVars) parseados a partir do
// conteúdo, escolhendo o extrator conforme a extensão.
func extractStructured(rel string, src []byte) (symbols, imports, calls, tables, envVars []string) {
	...
	case ".sql":
		...
		return nil, nil, nil, tables, nil
	default:
		return nil, nil, nil, nil, nil
	}
	calls = extractCalls(src)
	tables = extractTables(src)
	envVars = extractEnvVars(src)
	symbols = dedup(symbols)
	imports = dedup(imports)
	calls = dedup(calls)
	tables = dedup(tables)
	envVars = dedup(envVars)
	return
}
...
var (
	reSQLCreateTable = regexp.MustCompile(`(?i)\b(?:CREATE|ALTER|DROP)\s+TABLE\s+(?:IF\s+(?:NOT\s+)?EXISTS\s+)?["'` + "`" + `]?([a-zA-Z0-9_]+)["'` + "`" + `]?`)
	reSQLQueries     = regexp.MustCompile(`(?i)\b(?:FROM|JOIN|INTO|UPDATE)\s+["'` + "`" + `]?([a-zA-Z0-9_]+)["'` + "`" + `]?`)
	reEnvGo          = regexp.MustCompile(`(?i)\bos\.(?:Getenv|LookupEnv)\(["']([a-zA-Z0-9_]+)["']\)`)
	reEnvJS          = regexp.MustCompile(`(?i)\bprocess\.env(?:\.([a-zA-Z0-9_]+)|\[["']([a-zA-Z0-9_]+)["']\])`)
	reEnvPyPHP       = regexp.MustCompile(`(?i)\b(?:os\.environ(?:\.get)?\(|getenv\(|\$_ENV\[)["']([a-zA-Z0-9_]+)["']`)
	reEnvShell       = regexp.MustCompile(`\$\{?([A-Z0-9_]{3,})\}?`)
)

var sqlKeywords = map[string]bool{
	"where": true, "select": true, "set": true, "values": true, "order": true,
	"group": true, "having": true, "limit": true, "offset": true, "and": true,
	"or": true, "not": true, "in": true, "is": true, "null": true, "default": true,
	"primary": true, "foreign": true, "key": true, "constraint": true, "table": true,
	"index": true, "on": true, "as": true, "case": true, "when": true, "then": true,
	"else": true, "end": true, "distinct": true, "left": true, "right": true,
	"inner": true, "outer": true, "cross": true,
}

func extractTables(src []byte) []string {
	var tables []string
	str := string(src)
	for _, m := range reSQLCreateTable.FindAllStringSubmatch(str, -1) {
		name := strings.ToLower(m[1])
		if !sqlKeywords[name] && len(name) > 1 {
			tables = append(tables, m[1])
		}
	}
	for _, m := range reSQLQueries.FindAllStringSubmatch(str, -1) {
		name := strings.ToLower(m[1])
		if !sqlKeywords[name] && len(name) > 1 {
			tables = append(tables, m[1])
		}
...
func extractEnvVars(src []byte) []string {
	var envs []string
	str := string(src)
	for _, m := range reEnvGo.FindAllStringSubmatch(str, -1) {
		if m[1] != "" {
			envs = append(envs, m[1])
		}
	}
	for _, m := range reEnvJS.FindAllStringSubmatch(str, -1) {
		if m[1] != "" {
			envs = append(envs, m[1])
		} else if m[2] != "" {
	...
	for _, m := range reEnvPyPHP.FindAllStringSubmatch(str, -1) {
		if m[1] != "" {
			envs = append(envs, m[1])
		}
	}
	for _, m := range reEnvShell.FindAllStringSubmatch(str, -1) {
		if m[1] != "" && strings.ToUpper(m[1]) == m[1] {
			envs = append(envs, m[1])
		}
	}
	return dedup(envs)
...
func calculateBlastRadius(cache *Cache, targetPath string, importers []string, tables []string) (string, int) {
	impactCount := len(importers)
	if len(tables) > 0 {
		tblMap := make(map[string]bool)
		for _, t := range tables {
			tblMap[t] = true
		}
		for p, entry := range cache.Files {
			if p == targetPath {
				continue
			}
	...
	if impactCount >= 8 || base == "main.go" || base == "store.go" || base == "config.go" {
		return "high", impactCount
	}
	if impactCount >= 3 {
		return "medium", impactCount
	}
	return "low", impactCount
}

func detectTestCommand(targetPath string, tests []string) string {
	ext := strings.ToLower(filepath.Ext(targetPath))
	switch ext {
	case ".go":
		dir := filepath.Dir(targetPath)
		if strings.HasPrefix(targetPath, "tools/") {
			sub := strings.TrimPrefix(dir, "tools/")
			if sub == "tools" || sub == "." {
				return "cd tools && go test ./..."
			}
			return fmt.Sprintf("cd tools && go test ./%s/...", sub)
		}
		if dir == "." {
			return "go test ./..."
		}
	...
	case ".ts", ".tsx", ".js", ".jsx":
		if len(tests) > 0 {
			return fmt.Sprintf("npm test -- %s", tests[0])
		}
		return "npm test"
	case ".py":
		if len(tests) > 0 {
			return fmt.Sprintf("pytest %s", tests[0])
		}
		return "pytest"
	default:
		if len(tests) > 0 {
			return fmt.Sprintf("test %s", tests[0])
		}
		return ""
	}
}

func truncateBriefPacket(p *BriefPacket, maxTokens int) []byte {
	data, _ := json.MarshalIndent(p, "", "  ")
	if maxTokens <= 0 || len(data)/4 <= maxTokens {
		return data
	}
	if len(p.Calls) > 8 {
		p.Calls = append(p.Calls[:8], fmt.Sprintf("... (%d mais)", len(p.Calls)-8))
	}
	data, _ = json.MarshalIndent(p, "", "  ")
	if len(data)/4 <= maxTokens {
		return data
	}
	if len(p.Importers) > 8 {
		p.Importers = append(p.Importers[:8], fmt.Sprintf("... (%d mais)", len(p.Importers)-8))
	}
	data, _ = json.MarshalIndent(p, "", "  ")
	if len(data)/4 <= maxTokens {
		return data
	...
	if len(p.Symbols) > 10 {
		p.Symbols = append(p.Symbols[:10], fmt.Sprintf("... (%d mais)", len(p.Symbols)-10))
	}
	data, _ = json.MarshalIndent(p, "", "  ")
	return data
}
...
// BriefPacket representa um contexto estruturado e delimitado em tokens
// pronto para consumo de agentes LLM antes de planejar ou editar.
...
// Brief gera um resumo compacto em JSON com teto de tokens em torno de um arquivo.
func Brief(cache *Cache, targetPath string, maxTokens int) string {
	if cache == nil || len(cache.Files) == 0 {
		return `{"error": "cache vazio"}`
	}
	targetPath = normalizePath(targetPath)
	entry := cache.Files[targetPath]
	if entry == nil {
	...
	if entry == nil {
		return fmt.Sprintf(`{"error": "arquivo %s não encontrado no cache"}`, targetPath)
	}
	...
	for p := range cache.Files {
		if strings.Contains(p, base) && (strings.HasSuffix(p, "_test.go") || strings.Contains(p, "test") || strings.Contains(p, "Test")) {
			if p != targetPath {
	...
	sort.Strings(tests)
	testCmd := detectTestCommand(targetPath, tests)
	...
	data := truncateBriefPacket(&packet, maxTokens)
	return string(data)
}
...
// FindSymbolCallers devolve todos os arquivos onde o símbolo informado aparece em Calls.
func FindSymbolCallers(cache *Cache, symbol string) []string {
	if cache == nil || symbol == "" {
		return nil
	}
	var callers []string
	...
	for path, e := range cache.Files {
		for _, call := range e.Calls {
			if call == symbol || strings.HasSuffix(call, "."+symbol) {
				if !seen[path] {
					seen[path] = true
