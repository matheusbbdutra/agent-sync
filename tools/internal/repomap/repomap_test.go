...
`)
	syms, imps, calls := extractStructured("sample.go", src)
...
`)
	_, imps, _ := extractStructured("main.go", src)
...
def calculate(x):
    return x + 1
`)
	syms, imps, _ := extractStructured("svc.py", src)
...
`)
	syms, imps, _ := extractStructured("repo.ts", src)
...
`)
	syms, imps, _ := extractStructured("user.php", src)
